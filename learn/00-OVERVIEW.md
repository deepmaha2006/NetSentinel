...
- **Basic C++ programming** - You need familiarity with classes, smart pointers (`std::shared_ptr`), and lambda functions. This project uses C++20 features.
...
- **Command line comfort** - You'll compile with CMake and run the scanner from the terminal.

**Tools you'll need:**

- **CMake 3.15+** - Build system for C++ projects.
- **C++20 compiler** - GCC 10+, Clang 12+, or MSVC 2022+.
- **Boost libraries** - Boost.Asio and Boost.Program_options. Install with `apt install libboost-all-dev` or `brew install boost`.
...
Expected output: A table showing port number, state (OPEN/CLOSED/FILTERED), service name if recognized, and any banner grabbed from the service. Open ports appear in green, closed in red, filtered in yellow.
...
└── learn/
    ...
    ├── 01-CONCEPTS.md       # Security theory
    ├── 02-ARCHITECTURE.md   # System design
    ├── 03-IMPLEMENTATION.md # Code walkthrough
    └── 04-CHALLENGES.md     # Extensions and exercises
```
...
- Windows: Use vcpkg — `vcpkg install boost-asio boost-program-options boost-system`

**"Connection refused" on all ports**
Solution: This is normal if scanning a machine with no services running. Try `scanme.nmap.org` which has intentional open ports for testing, or scan your own machine after starting a service (`python3 -m http.server 8000`).

**Scanner hangs or runs very slowly**
Solution: Your firewall might be rate-limiting you. Reduce the thread count (`-t 10` instead of default 100) and increase timeout (`-e 5`).
